...
def get_accuracy(prediction, label):
    ...
    correct_predictions = predicted_classes.eq(
        label_classes
    ).sum()  # predicted_classes.eq(label).sum() #########
...
def train_and_test_model_with_hparams(
    data, hparams, model_type="lstm", distil=None, **kwargs
):  #
    ...
    (
        ...
        y_valid,
        y_test,
    ) = load_data_train_split.load_data_train_split(data)
    ...
    train_data = Text_Tokenize.Text_Tokenize(
        x_train, y_train, vocab, hparams.MAX_LENGTH
    )
    valid_data = Text_Tokenize.Text_Tokenize(
        x_valid, y_valid, vocab, hparams.MAX_LENGTH
    )
    ...
    if hparams.OPTIM == "sgd":
        optimizer = optim.SGD(
            model.parameters(), lr=hparams.LR, weight_decay=hparams.WD, momentum=0.9
        )
