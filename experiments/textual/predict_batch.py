...
def s_prediction_batch(student_model, data):
    ...
    model = (
        student_model  # torch.load(os.path.join(CHECKPOINT_FOLDER,f'{teacher_model}'))
    )
    model.eval()
...
def t_prediction_batch(teacher_model, old_data, new_data):
    _, _, x_test_new, _, _, y_test_new = load_data_train_split.load_data_train_split(
        new_data
    )
